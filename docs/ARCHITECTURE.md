...
- **Explicit, confirmed overrides — Clients and Assets, so far**: for these two top-level entities specifically, the product owner has explicitly asked for the slide-in-panel treatment instead of a real page, for both create and edit.
  ...
  - **Activities**: 2026-08-28 feedback ("Edit activity pagina, maar ook de new activity pagina is niet lekker ingedeeld... gebruik hier maar slider popup voor") asked for the same treatment a third time. `ActivityFormPanel` (`app/(app)/activities/components/activity-form-panel.tsx`, a `Dialog` `size="panel"`) replaced the old full-page `/activities/new` and `/activities/[id]/edit` routes (both deleted, along with that page's form component, `activity-form.tsx`). Same self-fetching-on-open shape as `AssetFormDialog` (an outer component owning `useActionState`, an inner body — remounted fresh every open — owning every other piece of local state and a mount effect that fetches every picklist/locked-record it needs), so every trigger (`CreateActivityButton`, `ActivitiesTable`'s row Edit, `ActivityQuickViewDialog`'s Edit button) stays a thin trigger with local `open` state, no page-level prop-threading required.
