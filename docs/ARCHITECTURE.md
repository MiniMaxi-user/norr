...
- **Breadcrumb the hierarchy**, don't just "back link" it. `BackLink` (single hop, "back to X") is right for a page with one obvious parent list. Once a page sits two or more levels deep in a real hierarchy (Clients → Acme Corp → Site X), it needs a breadcrumb trail showing the full path, not just a single hop backward — add a `Breadcrumbs` primitive to `@yourorg/ui` if one doesn't exist yet rather than approximating it with more `BackLink`s.
  - **Breadcrumbs render in the Topbar, not inline in the page body.** The mechanism is `usePageHeader()` (`components/shell/page-header-context.tsx`): a page calls `usePageHeader(<Breadcrumbs items={...} />)` (the element built with `useMemo`, keyed on its real inputs — see that hook's own doc comment for why an un-memoized element causes an infinite render loop) and it renders in `Topbar`'s title slot instead of scrolling away with `<main>`. `client-detail.tsx` and `clients-explorer.tsx` (kanban view only) are wired this way today; other detail pages (`assets/[id]/page.tsx`, work orders, contracts, quotes, ...) still render their own inline `Breadcrumbs` — not yet brought in line with this mechanism, but any *new* page should use `usePageHeader()` from the start rather than adding another inline instance. A record-editing form that's a slide-in panel (see "Popup vs. full page" above) never needs this at all — it isn't a page, there's nothing to breadcrumb.
...
- **Work Orders — a third variant of "real page," not a separate route per state (issue #89, "New workorder en edit workorder scherm gelijktrekken"):** `/work-orders/new` is still its own route (create genuinely has no id yet), but the old separate `/work-orders/[id]/edit` route is gone — an existing work order's own fields (title, client/site/asset/contract, assignment, schedule, status/priority) are inline-editable directly on the detail page (`app/(app)/work-orders/[id]/page.tsx`, via `WorkOrderFields`'s `mode: "edit"` in a `DetailLayout` rail), right alongside its Time Entries/Checklist sections — never a navigation away to a dedicated edit screen. `WorkOrderFields` renders those same fields read-only (plain text/links, not disabled inputs) for a caller without `update`/`update_own` on `planning`, so the one page serves both a viewer and an editor without a second route. The same story also converted Time Entries' "Add"/edit from `Dialog` popups to inline editable table rows (`time-entries-panel.tsx`) for the same "avoid an unnecessary popup/route hop" reasoning. Treat this as the pattern for a top-level entity whose detail page already has enough room for its own fields, not as license to silently retrofit Contracts/Quotes the same way without similar product-owner framing.
