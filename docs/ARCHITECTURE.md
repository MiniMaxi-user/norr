...
- `clients`, `sites`, `assets` — **implemented** (issue #7) in `supabase/migrations/20260822190000_clients_sites_assets.sql`, tested in `supabase/tests/database/clients_sites_assets_rls.test.sql`:
  ...
  - `assets (id, organization_id, client_id, site_id, name, type, manufacturer, model, serial_number, status asset_status['active'|'decommissioned'], installed_at date, warranty_until date, notes, created_by, created_at, updated_at)` — equipment installed at a site.
