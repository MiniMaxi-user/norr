 ...
 * relations. Issue #127 briefly widened it to also cover Contract, but issue
 * #128 (same-day reversal) removed that again: an activity's Contract is now
 * DERIVED from whichever asset (or client) it's linked to
 * (`useActivityContractCoverage`/`clientScoped.contracts` in `ActivityHero`),
 * never a separately picked FK — see that card's own comment in
 * `activity-hero.tsx`. Type used to live in this same dialog but now has its
 * own always-visible "Type" section (`ActivityTypeSection`, an `IconTileSelect`
