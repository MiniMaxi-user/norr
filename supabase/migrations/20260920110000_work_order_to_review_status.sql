-- Work Order "To review" status: schema-only prerequisite for an office
-- review gate between an engineer finishing a work order in the PWA and it
-- being considered fully `completed`. Today the PWA's "Finish workitem"
-- action sets a work order straight to `completed` when the engineer sends
-- it back from the field; product wants an intermediate "needs office
-- review" gate first. This migration only seeds/backfills the new
-- `to_review` item into the `work_order_status` reference list — no status
-- transition logic and no new SECURITY DEFINER function (unlike `checkout`'s
-- `checkout_work_order_if_scheduled`), since the transition into and out of
-- `to_review` will be a plain RLS-scoped update from the app layer. That
-- follow-up (NOT part of this migration, done by api-backend-engineer right
-- after this lands):
--   - the PWA finish route sets status to `to_review` instead of `completed`
--   - a narrow "Goedkeuren"/Approve action flips `to_review` -> `completed`
--
-- New lifecycle:
--   New -> Scheduled -> Checkout -> En Route -> In Progress -> To review ->
--   Completed -> Invoiced
-- `sort_order` renumbered accordingly (`completed`/`invoiced` shift up by 1),
-- for both `seed_default_reference_lists` (future organizations) and a
-- one-time backfill for every organization that already existed.
--
-- Design notes:
--
...
--    `20260920100000_work_order_checkout_status.sql` is still the true latest
--    redefinition of `seed_default_reference_lists` by grepping every
--    migration for `create or replace function
...
--    `work_order_status` block's insert values list gets a new `to_review`
--    row inserted in lifecycle position (sort_order 6), with `completed`/
--    `invoiced`'s `sort_order` values shifted from 6/7 to 7/8.
-- 2. **Existing-organization backfill renumbers `sort_order` too, not just
--    inserts the new item.** Same as `checkout` before it: `to_review` is
--    inserted in the MIDDLE of an already-seeded, already-ordered lifecycle
--    list, so the backfill loop below explicitly UPDATEs each existing
--    organization's `completed`/`invoiced` rows to their new `sort_order`
--    (scoped by `value = ... and sort_order = <the exact prior default>`, so
--    — same "never clobber a tenant's own customization" discipline as
--    `ensure_reference_list_defaults`/the `checkout` backfill — an
...
--    the new `to_review` row at `sort_order = 6`.
-- 3. **Label is Dutch (`Te controleren`), not English, by deliberate
--    exception.** Every other item in this list uses an English label
--    (`Checked out`/`En Route`/etc., per
--    `20260919090000_translate_default_reference_list_labels_to_english.sql`)
--    — this one is Dutch per explicit product instruction for this specific
--    item, matching this org's current UI language for this list. Value slug
--    (`to_review`) is still English, per this codebase's standing rule that
--    code/DB identifiers are always English regardless of label language.
-- 4. **No new SECURITY DEFINER transition function.** Unlike `checkout`
--    (`checkout_work_order_if_scheduled`), no automatic status transition is
--    needed for `to_review` — the PWA finish route and the follow-up
--    "Goedkeuren"/Approve action will both do plain RLS-scoped updates from
--    the app layer (api-backend-engineer, follow-up work, not part of this
--    migration).

-- ---------------------------------------------------------------------------
-- Part 1: seed_default_reference_lists — `to_review` inserted into
-- `work_order_status` at lifecycle position 6 (design note 1 above). Full
-- body copied forward from 20260920100000_work_order_checkout_status.sql
...
begin
  ...
  -- In Progress -> To review -> Completed -> Invoiced. `to_review` added by
  -- the "office review gate" change: the PWA's finish action will send a
  -- work order here instead of straight to `completed`, and a narrow
  -- "Goedkeuren"/Approve action flips it to `completed` (both a follow-up
  -- outside this migration). Label is deliberately Dutch (`Te controleren`),
  -- unlike every other item in this list — see this migration's header
  -- design note 3.
  insert into public.reference_lists (organization_id, list_key, name)
  ...
  values
    ...
    (v_work_order_status_list_id, p_organization_id, 'to_review', 'Te controleren', 6, false),
...
-- ALREADY-SEEDED work_order_status items to make room for `to_review` at
-- position 6 (design note 2 above). Each UPDATE is scoped by
...
-- ensure_reference_list_defaults / the `checkout` backfill migration.
-- Must run AFTER part A (so every organization's work_order_status list is
-- guaranteed to already contain the new `to_review` row by the time this
